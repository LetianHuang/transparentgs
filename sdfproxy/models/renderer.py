...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    pdf = weights / torch.sum(weights, -1, keepdim=True)#概率
...
class NeuSRenderer:
    ...
    def up_sample_occulsion(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
        ...
        e_sdf = torch.exp(-mid_sdf*inv_s)
        density = (e_sdf/torch.pow(1+e_sdf, 2))* inv_s * dist

        z_samples = sample_pdf(z_vals, density, n_importance, det=True).detach()
    ...
    def render_core(self,
        ...
        gradients = ((gradients * weights[:, :gradients.shape[1], None]) * inside_sphere[..., None]).sum(dim=1)

        inter_point = pts.reshape(batch_size, n_samples, 3)
        sdf = sdf.reshape(batch_size, n_samples)
        inter_point = ((inter_point * weights[:, :inter_point.shape[1], None]) * inside_sphere[..., None]).sum(dim=1)
    ...
    def check_occlusion(self, check_sdf):
        ...
        with torch.no_grad():
            ...
            # valid_mask = (valid_mask * weight_sum2.clip(1e-3, 1.0 - 1e-3)[:, 0] > 0.8)
