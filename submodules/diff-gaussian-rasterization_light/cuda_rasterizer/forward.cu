...
__device__ float2 computeTangentGaussian(const float2 xy, const float2 pixf, const int H, const int W, const float pi_invh, const float pi_invw, bool * flag)
{
	...
	const bool is_train)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&(out_trans[collected_id[j]]), T * alpha);
			atomicAdd(&(non_trans[collected_id[j]]), alpha);
