...
__device__ float3 computeNaiveCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix)
{
	...
	const float x2 = t.x * t.x;
	const float y2 = t.y * t.y;
	const float z2 = t.z * t.z;
	
	const float x2y2 = x2 + y2;
	const float x2y2z2 = x2y2 + z2;

	const float x2y2_0_5 = pow(x2y2, 0.5f);
	const float x2y2_1_5 = pow(x2y2, 1.5f);
	const float x2y2_2_0 = pow(x2y2, 2.0f);
	const float x2y2_2_5 = pow(x2y2, 2.5f);
	const float x2y2_3_0 = pow(x2y2, 3.0f);

	const float at = atan2(x2y2_0_5, t.z);
	
	const float a_1_0 = x2y2 * x2y2z2;
	const float a_1_5 = x2y2_1_5 * x2y2z2;
	const float a_2_5 = x2y2_2_5 * x2y2z2;
	const float a_3_0 = x2y2_3_0 * x2y2z2;

	const float AB = t.x * t.y * (t.z * x2y2_2_5 - a_1_0 * at) / (a_2_5 + 0.0000001f);



	glm::mat3 J = glm::mat3(
		focal_x * (x2 * t.z * x2y2_2_0 - x2 * a_1_5 * at + a_2_5 * at) / (a_3_0 + 0.0000001f), 
		focal_x * AB,
		-focal_x * t.x / (x2y2z2 + 0.0000001f),

		focal_y * AB,
		focal_y * (y2 * t.z * x2y2_2_0 - y2 * a_1_5 * at + a_2_5 * at) / (a_3_0 + 0.0000001f), 
		-focal_y * t.y / (x2y2z2 + 0.0000001f),
		0, 0, 0);
...
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix)
{
	...
	const float fx = focal_x;
	const float fy = focal_y;
	...
	bool prefiltered)
{
	...
	/**
	 * ------------------------------- For Optimal Gaussian Splatting ------------------------------------------
	*/
	...
	*/
	...
	/**
	 * ------------------------------- For Fish Gaussian Splatting ------------------------------------------
	*/
	const float cx = 0.5f * W - 0.5f;
	const float cy = 0.5f * H - 0.5f;
	const float pi = 3.1415926f;
	const float fx = focal_x;
	const float fy = focal_y;
	float theta = atan2(sqrt(p_proj.x * p_proj.x + p_proj.y * p_proj.y), p_proj.z);

	float phi = atan2(p_proj.y, p_proj.x);
  ...
  // thanks to [Hanson et al. 2025] Speedy-Splat: Fast 3D Gaussian Splatting with Sparse Pixels and Sparse Primitives. CVPR 2025
	...
	float focal_x, float focal_y)
{
	...
	const float fx = focal_x;
	const float fy = focal_y;
	const float inv_focal_x = 1.0f / fx;
	const float inv_focal_y = 1.0f / fy;

	float ux = (pixf.x - cx) * inv_focal_x;
	float vy = (pixf.y - cy) * inv_focal_y;

	float phi = atan2(vy, ux);
	float theta = sqrt(ux*ux + vy*vy);

	float Th = H / (2.0f * focal_y);
	if (theta >= Th || theta <= -Th) {
		inside = false;
		done = true;
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			ux = (xy.x - cx) * inv_focal_x;
			vy = (xy.y - cy) * inv_focal_y;
			phi = atan2(vy, ux);
			theta = sqrt(ux*ux + vy*vy);
