...
from utils.point_utils import depths_to_points, CameraTempView, depth_to_normal
...
def gbuffer_pass(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    GRSetttings=diff_surfel_rasterization.GaussianRasterizationSettings
    GRasterizer=diff_surfel_rasterization.GaussianRasterizer
    ...
    # print(f"means3D: {means3D.shape}, means2D: {means2D.shape}, shs: {shs.shape}, opacity: {opacity.shape}, scales: {scales.shape}, rotations: {rotations.shape}")
    ...
    H, W = surf_depth.shape[1:]
    surf_points = depths_to_points(viewpoint_camera, surf_depth).reshape(H, W, 3).permute(2, 0, 1)
    # print(f"surf_depth.shape: {surf_depth.shape}, surf_points.shape: {surf_points.shape}")
