...
def cubemap_to_latlong_c2w(cubemap, res,c2w):
    gy, gx = torch.meshgrid(torch.linspace( 0.0 + 1.0 / res[0], 1.0 - 1.0 / res[0], res[0], device='cuda'), 
                            torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device='cuda'),
                            # indexing='ij')
                            )
...
import cv2
def load_hdr_win2(fn) -> np.ndarray:
    data = cv2.imread(fn, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    if data is None:
        raise ValueError("OpenCV dont recognize EXR")
