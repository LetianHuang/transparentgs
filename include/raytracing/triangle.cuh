...
struct Triangle {
    ...
    __host__ __device__ void get_normals(Eigen::Vector3f n[3]) const {
        n[0] = na;
        n[1] = nb;
        n[2] = nc;
    }

    __host__ __device__ Eigen::Vector3f get_interp_normal(Eigen::Vector3f point) const {
        Eigen::Vector3f v0 = b - a, v1 = c - a, v2 = point - a;
        float d00 = v0.dot(v0);
        float d01 = v0.dot(v1);
        float d11 = v1.dot(v1);
        float d20 = v2.dot(v0);
        ...
        Eigen::Vector3f interp_normal = (u * na + v * nb + w * nc).normalized();
